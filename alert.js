...
export const setupTwilioClient = () => twilio(env.TWILIO_ACCOUNT_SID, env.TWILIO_AUTH_TOKEN);
...
export const sendTextAlert = async (client, logger) => {
  await Promise.map(env.PHONE_NUMBERS.split(','), async (number) => {
    ...
    const res = await client.messages
      .create({ body: `PS5 ALERT!!! visit the following link ASAP: ${targetURL}`, from: env.TWILIO_PHONE_NUMBER, to: number })
